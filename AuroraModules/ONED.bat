...
for /f "usebackq tokens=2 delims=\" %%e in (`reg query "HKEY_USERS" ^| findstr /r /x /c:"HKEY_USERS\\S-.*" /c:"HKEY_USERS\\ibrhub_UserHive_[^_]*"`) do (
	REM If the "Volatile Environment" key exists, that means it is a proper user. Built in accounts/SIDs do not have this key.
	reg query "HKU\%%e" | findstr /c:"Volatile Environment" /c:"ibrhub_UserHive_" > nul 2>&1
	if not errorlevel 1 (
		call :USERREG "%%e"
	)
)
...
if not exist "%SystemRoot%\System32\OneDriveSetup.exe" (
    echo OneDrive is not installed. Exiting...
)

if not exist "%SystemDrive%\Users\*\AppData\Local\Microsoft\OneDrive" (
    echo OneDrive is not installed for any user. Exiting...
    exit /b 0
)
